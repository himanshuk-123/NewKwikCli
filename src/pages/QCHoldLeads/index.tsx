...
import { fetchQcHoldLeadsApi } from "../../features/qcHoldLeads/api/qcHoldLeads.api";
      ...
      <View style={styles.cardRow}>
        <View style={styles.topLeftComponent}>
          ...
          <View style={styles.remarkRow}>
            ...
            <Text style={[styles.textBold, styles.remarkText]}>{(lead.LeadRemark) || "NA"}</Text>
          </View>
        </View>
...
export default function QCHoldLeads() {
  const [qcHoldLeads, setQcHoldLeads] = useState<LeadListStatuswiseRespDataRecord[]>([]);
  ...
  const loadData = useCallback(() => {
    ...
    fetchQcHoldLeadsApi()
      ...
      .catch((error) => {
        console.error("[QCHoldLeads] Fetch error:", error);
        if (isActive) {
          setQcHoldLeads([]);
        }
      })
